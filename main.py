...
class CSVProcessorApp:
    ...
    def run(self):
        ...
        try:
            ...
            if args.where:
                column, operator, value = self.processor.parse_where_condition(
                    args.where
                )
            ...
            if args.aggregate:
                column, operation = self.processor.parse_aggregate_condition(
                    args.aggregate
                )
                result = self.processor.apply_aggregation(
                    filtered_data, column, operation
                )
